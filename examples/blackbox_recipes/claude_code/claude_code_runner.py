...
def resolve_gateway_proxy_port(proxy_port: int | str | None) -> int:
    if proxy_port is not None:
        return validate_gateway_proxy_port(proxy_port, source="runner_kwargs.proxy_port")

    raw = os.getenv("CLAUDE_CODE_PROXY_PORT")
    if raw is None or raw.strip() == "":
        return DEFAULT_GATEWAY_PROXY_PORT
    return validate_gateway_proxy_port(raw, source="CLAUDE_CODE_PROXY_PORT")
...
async def claude_code_runner(
    ...
    **kwargs,
    ...
    gateway_proxy_port = resolve_gateway_proxy_port(proxy_port)
